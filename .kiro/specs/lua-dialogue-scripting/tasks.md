...
- [x] 1. lupa 라이브러리 설치 및 프로젝트 의존성 추가
  - [x] 1.1 requirements.txt에 `lupa>=2.0` 추가 및 pip install 실행
    - requirements.txt의 Core runtime packages 섹션에 lupa 추가
    - 가상환경에서 `pip install lupa` 실행하여 설치 확인
    - _Requirements: 1.1, 1.2_

- [x] 2. LuaScriptLoader 클래스 구현
  - [x] 2.1 `src/mud_engine/game/lua_script_loader.py` 파일 생성
...
- [x] 3. DialogueContext 클래스 구현
  - [x] 3.1 `src/mud_engine/game/dialogue_context.py` 파일 생성
...
- [x] 4. Checkpoint - 기반 컴포넌트 검증
  - mypy + ruff 정적 검사 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. DialogueInstance 수정 (Lua 연동)
  - [x] 5.1 `src/mud_engine/game/dialogue.py` 수정
...
- [x] 6. DialogueManager 수정 (다국어 + LuaScriptLoader 초기화)
  - [x] 6.1 `src/mud_engine/game/managers/dialogue_manager.py` 수정
...
- [x] 8. 샘플 Veteran Guard Lua 스크립트 작성
  - [x] 8.1 `configs/dialogues/3914fbe8-c8a9-493a-b451-1084ee4d6d2a.lua` 파일 생성
...
- [x] 9. Telnet MCP E2E 테스트
  - [x] 9.1 서버 실행 후 Telnet MCP로 Veteran Guard 대화 전체 흐름 테스트
