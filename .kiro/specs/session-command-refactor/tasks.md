...
## 페이즈1 종료 기록 (2026-08-08)

이 스펙은 Task 1, 2, 3.1~3.4를 완료한 상태에서 페이즈1 종료와 함께 닫혔다. 미진행 항목(3.5, 4, 5, 6, 7, 8)의 체크박스는 이력 보존을 위해 그대로 남겨 두었으며, 앞으로 이 스펙에서 착수하지 않는다.

방향 전환 사유는 클라이언트 아키텍처 변경이다. 페이즈2에서 플레이어 접속 경로가 Telnet 텍스트에서 Godot GUI 클라이언트로 바뀌고, 서버는 사람이 읽는 텍스트를 만들지 않으며 개행 구분 JSON 라인만 내보낸다. 이에 따라 본 스펙의 남은 작업 대부분이 전제를 상실했다.

미진행 항목별 처리:

| 항목 | 처리 |
|---|---|
| 3.5 Presenter 분리 | 대체됨. Presenter를 분리하는 대신 `_format_message`/`_format_room_info`와 `ansi_colors.py`를 제거한다. 팩션 판정의 `FactionManager` 위임은 페이즈2로 이전 |
| 4.1 OutputPort/CommandContext | 대체됨. 출력은 JSON 라인 송출로 단일화된다 |
| 4.2 CommandResult 출력 책임 제거 | 흡수됨. 페이즈2에서 `message: str`을 `message_key`/`params`로 교체한다 |
| 4.3 일괄 전송 폐지 | 대체됨. 텍스트 누적 자체가 사라진다 |
| 5 명령어 디렉토리 표준화 | 방향 변경. 완전 버튼 기반 UI로 타이핑이 사라지므로 문자열 파싱과 별칭 체계(n/s/e/w, att/kill/fight 등)를 폐기하고 액션 디스패처로 재작성한다 |
| 6.3 숫자 입력 매핑 데이터화 | 폐기. 대상 지정이 uuid 전용으로 바뀌어 idx 체계 자체가 사라진다. 전투 액션은 verb로, 대화 선택지는 params로 전달한다 |
| 6.1 6.2 등록 일원화 | 페이즈2 액션 디스패처 설계에 흡수 |
| 7 Processor 정책 정리 | 페이즈2 액션 디스패처 설계에 흡수 |
| 8 최종 회귀 검증 | 대체됨. telnet-mcp 기반 회귀 대신 `scripts/`의 JSON 라인 하니스로 검증한다 |

후속 스펙은 `.kiro/specs/server-json-protocol/`이다. 프로토콜 계약은 `docs/protocol/`을 참조한다.
