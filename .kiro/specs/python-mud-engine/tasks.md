...
- [ ] 22. 클라이언트 고도화
  ...
  - 웹 UI에 명령어 조합 패널 추가 (클릭 기반)
  - 키보드 단축키와 버튼 조합 시스템 통합
  - _요구사항: 8.2, 8.3, 8.4, 8.5_

- [ ] 23. 전투 시스템 구현

  - **몬스터 데이터 모델**: Monster 클래스 및 DB 스키마 추가 (HP, 공격력, 방어력, 경험치, 드롭 아이템)
  - **몬스터 스폰 시스템**: 특정 지역에 고정 스폰 포인트 및 리스폰 타이머 구현
  - **몬스터 AI 시스템**: 로밍형(주변 이동) vs 고정형(제자리 대기) 행동 패턴
  - **어그로 시스템**: 어그레시브형(선공) vs 패시브형(후공) 몬스터 타입 구현
  - **전투 메커니즘**: 간소화된 D&D 룰 기반 전투 계산 (공격/방어/회피/크리티컬)
  - **전투 명령어**: attack, flee, defend 명령어 구현
  - **전투 상태 관리**: 전투 중 이동 제한, 턴 기반 액션 처리
  - **경험치 및 레벨업**: 몬스터 처치 시 경험치 획득 및 능력치 상승 시스템
  - **전리품 시스템**: 몬스터 처치 시 아이템 드롭 및 자동 획득
  - **NPC 전투 시스템**: 기본적으로 NPC도 패시브형이지만 전투 가능
  - **우호도 시스템**: 경비병 타입과 같이 소속 진영에 대한 우호도에 따라 선공하는 NPC 구현
  - 웹 UI에 전투 상태 패널 추가 (HP/MP 바, 전투 로그, 액션 버튼)
  - _요구사항: 2.2, 2.3, 4.1, 4.2, 5.1, 6.1_
