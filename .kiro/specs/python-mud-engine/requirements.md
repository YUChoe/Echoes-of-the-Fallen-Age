...
- **PlayerStats**: D&D 기반 1차 능력치(STR/DEX/INT/WIS/CON/CHA)와 파생 스탯(HP/MP/STA/ATK/DEF/SPD/RES/LCK/INF)
- **Tutorial_Announcer**: 마을 광장의 신입 플레이어에게 튜토리얼 안내를 주기적으로 전송하는 컴포넌트 (TutorialAnnouncer)
...
**사용자 스토리:** 플레이어로서, 나는 D&D 기반 능력치 시스템을 통해 캐릭터의 강점을 확인할 수 있기를 원한다. 그래야 전투와 탐험에서 전략적 선택을 할 수 있다.

#### 승인 기준

1. THE Player 모델은 6개의 1차 능력치(strength, dexterity, intelligence, wisdom, constitution, charisma)를 1-100 범위로 저장해야 한다
2. WHEN 플레이어가 stats 명령을 사용하면 THEN Command_Processor는 1차 능력치와 파생 스탯(HP, MP, STA, ATK, DEF, SPD, RES, LCK, INF, max_carry_weight)을 계산하여 표시해야 한다
3. THE PlayerStats는 파생 스탯을 다음 공식으로 계산해야 한다: HP = 10 + (CON × 5), MP = 50 + (INT × 3) + (WIS × 2), STA = 100 + (CON × 3) + (DEX × 2), ATK = 10 + (STR × 2) + 장비보너스, DEF = 2 + (CON × 0.3) + 장비보너스, SPD = 10 + (DEX × 1.5), RES = 5 + (WIS × 1.5), LCK = 10 + (전체 능력치 평균 / 10), INF = 5 + (CHA × 2), max_carry_weight = 5 + (STR × 5)
...
6. THE 능력치 시스템은 레벨 개념을 포함하지 않으며, 캐릭터 강화는 장비와 1차 능력치 변경으로만 이루어진다
...
**사용자 스토리:** 플레이어로서, 나는 NPC와 대화하고 선택지를 통해 상호작용할 수 있기를 원한다. 그래야 게임 세계와 더 풍부하게 상호작용할 수 있다.

#### 승인 기준

1. WHEN 플레이어가 talk 명령과 NPC 번호(entity_map 인덱스)를 입력하면 THEN DialogueManager는 DialogueInstance를 생성하고 세션을 대화 모드로 전환해야 한다
2. WHEN 대화가 시작되면 THEN 세션의 in_dialogue=True, current_room_id를 dialogue_{id}로 변경하고, original_room_id에 원래 방 ID를 보존해야 한다
3. WHEN 대화 중 플레이어가 숫자를 입력하면 THEN DialogueInstance.get_dialogueby_choice()로 선택지를 처리하고 후속 대화를 표시해야 한다
4. WHEN NPC에 대화 스크립트(configs/dialogues/{npc_id}.lua)가 없으면 THEN "아무 말 없이 바라봅니다" 메시지와 [1] Bye 선택지를 표시해야 한다
5. WHEN 플레이어가 Bye 선택지를 선택하면 THEN DialogueManager.end_dialogue()가 세션을 원래 방으로 복원하고 대화 모드를 해제해야 한다
6. WHEN 대화가 종료되면 THEN session.in_dialogue=False, current_room_id를 original_room_id로 복원, dialogue_id=None으로 초기화해야 한다

#### 구현 구조

- DialogueInstance (src/mud_engine/game/dialogue.py): 대화 인스턴스 dataclass. interlocutor(NPC), player, session, choice_entity(선택지 맵) 관리
- DialogueManager (src/mud_engine/game/managers/dialogue_manager.py): 대화 인스턴스 생성/조회/종료, 메시지 전송
- TalkCommand (src/mud_engine/commands/dialogue/talk_command.py): talk 명령어 처리. 대화 시작 및 대화 중 선택지 입력 분기
- 기존 npc/talk_command.py는 deprecated 처리됨
...
2. THE Quest_Manager는 퀘스트 타입(TUTORIAL, MAIN, SIDE, DAILY)을 지원하고, 각 퀘스트는 다국어 이름/설명, 선행 퀘스트, 목표 목록, 보상을 포함해야 한다
...
6. THE Quest_Manager는 퀘스트 시작 가능 여부를 선행 퀘스트 완료 여부로 판단해야 한다
