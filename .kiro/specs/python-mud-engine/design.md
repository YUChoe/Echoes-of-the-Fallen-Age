...
전투는 독립적인 인스턴스로 관리되며, 각 인스턴스는 턴제 방식으로 진행됩니다.

#### 전투 인스턴스 특징
- **인스턴스 생성**: 전투 발생 시 독립적인 전투 인스턴스 생성
- **참가자 구분**: 전투 인스턴스에 참가한 플레이어와 참가하지 않은 플레이어 명확히 구분
- **턴제 시스템**: 인스턴스 내에서만 턴제로 진행
- **턴 순서**: 몬스터와 플레이어의 민첩(Agility) 수치 기반으로 턴 순서 결정
- **행동 선택**: 각 턴마다 플레이어는 행동(공격, 방어, 스킬, 도망 등) 선택

#### 공격적 몬스터 시스템
- **어그로 시스템**: 특정 몬스터는 "공격적(Aggressive)" 속성 보유
- **자동 전투 시작**: 플레이어가 공격적 몬스터가 있는 방에 입장 시
  - 몬스터가 전투 중이 아닌 경우 새 전투 인스턴스 생성
  - 해당 플레이어를 자동으로 전투에 참여시킴
- **전투 상태 관리**: 몬스터는 전투 중/비전투 상태 구분

#### 전투 흐름
1. 플레이어가 공격적 몬스터가 있는 방 입장
2. 몬스터가 비전투 상태인 경우 전투 인스턴스 생성
3. 민첩 수치 기반 턴 순서 결정
4. 각 턴마다 참가자는 행동 선택
5. 전투 종료 조건 충족 시 인스턴스 해제
...
- **메모리 관리**: WeakSet 사용
- **캐싱**: 자주 접근하는 방 데이터 캐싱
- **전투 인스턴스 관리**: 메모리 효율적인 전투 인스턴스 생성/해제
