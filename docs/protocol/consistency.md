...
| Lua 아이템 콜백 문장 미전달 | 미해결. Task 6.7에서 `result.message` 직송을 제거했다. `actions/items.py`의 `use`/`consume` 가 Lua 콜백의 완성 문장을 `ActionResult.message`로 실어 오는데, 이 필드는 개발자용 사유로 재정의됐으므로 서버 로그에만 남는다. Task 10은 대화 대사만 옮겼다. 남은 문장은 `configs/items/health_potion.lua` 와 `forgotten_scripture.lua` 두 건이며, 키로 바꾸는 것 외에 전달 경로도 정해야 한다. `ActionResult.message` 는 개발자용이므로 사용자 문장은 `event` 로 나가야 하고 그 카테고리 결정이 필요하다 |
