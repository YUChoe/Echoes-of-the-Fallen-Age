...
4001은 외부에 노출하지 않는다. 방화벽이나 바인드 주소로 접근을 제한하고 게이트웨이만 도달할 수 있게 구성한다.
...
토큰으로 붙는 서비스 주체가 있었다. 랜딩 사이트의 백엔드가 계정을 만들기 위한 것이었다. 계정 생성이 게임 채널의 `register`로 옮겨가면서 없앴다.
...
인증 전에 허용되는 메시지는 `admin_login`과 `ping`뿐이다. 그 외는 `admin_rejected`에 `NOT_AUTHENTICATED`로 응답하며 연결은 유지한다. 인증 시도 없이 60초가 지나면 연결이 끊어진다.
...
관리자가 계정을 만드는 경로다. 새 플레이어는 게임 채널의 `register`로 직접 만든다. 두 경로는 같은 검증 규칙을 쓰고 감사 기록과 응답 형식만 다르다.
