...
class NPC(BaseModel):
    ...
    def get_localized_name(self, locale: str = 'en') -> str:
        """로케일에 따른 NPC 이름 반환"""
        return self.name.get(locale, self.name.get('en', self.name.get('ko', 'Unknown NPC')))
    ...
    def get_dialogue_lines(self, locale: str = 'en') -> List[str]:
        """로케일에 따른 대화 대사 반환"""
        return self.dialogue.get(locale, self.dialogue.get('en', self.dialogue.get('ko', ['Hello!'])))
